  ...
  navigator.geolocation.getCurrentPosition(function(position) {
    ...
    document.addEventListener('touchstart', function(event) {
      if (event.target.classList.contains('reveal-button')) {
        var extractContainer = event.target.previousElementSibling;
        if (extractContainer.style.display === 'block') {
          extractContainer.style.display = 'none';
          event.target.textContent = 'Reveal Details';
          if (event.target.speechButton) {
            event.target.speechButton.parentNode.removeChild(event.target.speechButton);
            delete event.target.speechButton;
          }
        } else {
          ...
          event.target.speechButton = speechButton;
          // Set up the speech synthesis API
          var synth = window.speechSynthesis;
          var voices = synth.getVoices();
          speechButton.addEventListener('touchstart', function(event) {
